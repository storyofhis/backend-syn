...
import (
	...
	"github.com/storyofhis/category-svc/internal/config"
	"github.com/storyofhis/category-svc/internal/handler"
...
func runGRPCServer(grpcPort string, categoryService service.CategoryService) {
	listener, err := net.Listen("tcp", grpcPort)
	if err != nil {
		log.Fatalf("failed to listen: %v", err)
	}
	s := grpc.NewServer()
	pb.RegisterCategoryServiceServer(s, &handler.Server{
...
func runGatewayServer(grpcPort, httpPort string) {
	...
	err := pb.RegisterCategoryServiceHandlerFromEndpoint(context.Background(), mux, grpcPort, opts)
	if err != nil {
...
func main() {
	...
	bookService := service.NewCategoryService(categoryRepo)
